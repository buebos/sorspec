...
import (
	...
	"github.com/gael-herrera/sorspec/layer"
...
var initialize = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		*/
		for i := 0; i < len(requirements); i++ {
			...
			*/
			req := requirement.Requirements[requirements[i]]
		...
		fmt.Printf("Info: created sorspec project configuration under: '%s'\n", dir)
	},
	/*
	...
	*/
	Args: func(cmd *cobra.Command, args []string) error {
		...
		requirements, err := cmd.Flags().GetStringSlice("requirements")

		if err != nil {
			return fmt.Errorf("could not get requirements argument")
		}
		...
		for _, l := range layer.Layers {
			core, _ := cmd.Flags().GetString(l)

			/* The flag has default value and was not set by the user */
			if !cmd.Flag(l).Changed {
				continue
			}

			if layer.GetCoreLatest(l, core) == "" {
				return fmt.Errorf("no core named: '%s' for layer %s", core, l)
			}
		}
...
func init() {
	...
	initialize.Flags().StringSliceP("requirements", "r", nil, "a list of requirements your application will be needing")
}
